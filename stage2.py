...
def random_max(values, key):
    if not isinstance(values, tuple):
        values = tuple(values)
    max_key = max(map(key, values))  # Might throw ValueError
...
def update(board, move, symbol):
    ...
    try:
        best_Q = -max(Q.get((succ_board, action), 0)
...
def rl_player(symbol):
    ...
    while True:
        ...
        try:
            move = random_max(find_legal_moves(board),
                              key=lambda m: Q.get((board, m), 0))
